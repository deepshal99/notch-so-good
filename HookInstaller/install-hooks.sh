...
[{"matcher":"","hooks":[{"type":"command","command":"INPUT=$(cat); echo \"$INPUT\" | python3 -c \"\nimport sys, json, socket\nd = json.load(sys.stdin)\ntext = d.get('last_assistant_message','')\nif not text:\n    try:\n        f = open(d.get('transcript_path',''))\n        f.seek(0, 2)\n        size = f.tell()\n        f.seek(max(0, size - 200000))\n        chunk = f.read()\n        f.close()\n        for line in reversed(chunk.splitlines()):\n            try:\n                e = json.loads(line)\n            except:\n                continue\n            if e.get('type') == 'assistant':\n                c = e.get('message',{}).get('content',[])\n                if isinstance(c, list):\n                    for b in c:\n                        if isinstance(b, dict) and b.get('type') == 'text' and b.get('text','').strip():\n                            text = b.get('text','').strip()\n                            break\n                if text:\n                    break\n    except: pass\nif not text:\n    text = 'Task completed'\nmsg = json.dumps({'event':'Stop','session_id':d.get('session_id',''),'last_assistant_message':text[:200]})\ntry:\n    s = socket.socket(socket.AF_UNIX, socket.SOCK_STREAM)\n    s.connect('/tmp/notchsogood.sock')\n    s.sendall(msg.encode())\n    s.close()\nexcept: pass\n\"","timeout":5000}]}]
HOOKEOF

# --- Notification: Claude wants user attention ---
read -r -d '' NOTIFICATION_HOOK << 'HOOKEOF' || true
[{"matcher":"","hooks":[{"type":"command","command":"INPUT=$(cat); echo \"$INPUT\" | python3 -c \"\nimport sys, json, socket\nd = json.load(sys.stdin)\nnt = d.get('notification_type','')\nif not nt:\n    ml = d.get('message','').lower()\n    if 'permission' in ml or 'approval' in ml:\n        nt = 'permission_prompt'\n    elif 'waiting' in ml or 'input' in ml or 'idle' in ml:\n        nt = 'idle_prompt'\n    else:\n        nt = 'general'\nmsg = json.dumps({'event':'Notification','session_id':d.get('session_id',''),'notification_type':nt,'message':d.get('message','Claude needs attention')[:200],'title':d.get('title','')})\ntry:\n    s = socket.socket(socket.AF_UNIX, socket.SOCK_STREAM)\n    s.connect('/tmp/notchsogood.sock')\n    s.sendall(msg.encode())\n    s.close()\nexcept: pass\n\"","timeout":5000}]}]
HOOKEOF
